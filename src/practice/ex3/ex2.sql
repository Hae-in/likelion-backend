...
select TIMESTAMPDIFF(month, start_date, end_date) from job_history;

-- 3. `employees` 테이블에서 오늘로부터 입사한 지 30년이 된 직원들의 이름과 입사 날짜를 조회하세요.
select first_name, last_name, hire_date from employees where hire_date = date_sub(curdate(), interval 30 YEAR);

-- 4. `employees` 테이블에서 이번 달이 입사월인 직원들의 이름과 이메일을 조회하세요.
select first_name, last_name, email from employees where month(hire_date) = month(curdate());
...
select salary * 1200 from employees;

-- 4. `employees` 테이블에서 전체 직원의 평균 급여보다 높은 급여를 받는 직원들의 이름과 급여를 조회하세요.
select first_name, last_name, salary from employees where salary > ( select avg(salary) from employees);
select first_name, last_name, salary from employees join (select avg(salary) as avg_salary from employees) a on employees.salary > a.avg_salary;
...
select salary*12 as 연봉 from employees order by 연봉 desc;

-- 8. `job_history` 테이블에서 각 기록에 대해 부서 변경 횟수를 계산하세요.
부서 변경 횟수...????????????????? select * from job_history;
